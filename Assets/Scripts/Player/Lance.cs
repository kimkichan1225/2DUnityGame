...
// Lance.cs는 플레이어가 검을 장착했을 때 공격을 처리하는 스크립트입니다.
...
public class Lance : MonoBehaviour
{
    ...
    // 사운드 클립
    [SerializeField] private AudioClip Lanceattack1Sound; // Attack1 사운드 클립
    [SerializeField] private AudioClip Lanceattack2Sound; // Attack2 사운드 클립
    ...
    private void Update()
    {
        // 랜스가 장착되지 않았다면 공격 처리하지 않음
        if (playerController == null || !playerController.hasLance) return;
    ...
    public IEnumerator Attack1()
    {
        ...
        // Attack1 애니메이션 실행
        animator.SetTrigger("LanceAttack1");
        ...
        yield return new WaitForSeconds(0.6f); // 애니메이션 지속 시간 대기
    ...
    private IEnumerator Attack2()
    {
        ...
        // Attack2 애니메이션 실행
        animator.SetTrigger("LanceAttack2");
    ...
    public void PlayLanceAttack1Sound()
    {
        if (Lanceattack1Sound != null)
        {
            audioSource.PlayOneShot(Lanceattack1Sound);
        }
    }

    // Attack2 사운드 재생 (애니메이션 이벤트로 호출)
    public void PlayLanceAttack2Sound()
    {
        if ( Lanceattack2Sound != null && !audioSource.isPlaying)
        {
            audioSource.PlayOneShot( Lanceattack2Sound);
        }
    }
