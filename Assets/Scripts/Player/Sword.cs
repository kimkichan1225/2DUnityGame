...
// Sword.cs는 플레이어가 검을 장착했을 때 공격을 처리하는 스크립트입니다.
...
public class Sword : MonoBehaviour
{
    ...
    // 사운드 클립
    [SerializeField] private AudioClip Swordattack1Sound; // Attack1 사운드 클립
    [SerializeField] private AudioClip Swordattack2Sound; // Attack2 사운드 클립
    ...
    private void Update()
    {
        // 검이 장착되지 않았다면 공격 처리하지 않음
        if (playerController == null || !playerController.hasSword) return;
    ...
    public IEnumerator Attack1()
    {
        ...
        yield return new WaitForSeconds(0.8f); // 애니메이션 지속 시간 대기
    ...
    private IEnumerator Attack2()
    {
        ...
        yield return new WaitForSeconds(0.6f); // 애니메이션 동작 시간 대기

        // 콤보 종료
        canCombo = false;
        comboTimer = 0f;

        yield return new WaitForSeconds(0.35f); // 후딜레이 대기
    ...
    {
        if (Swordattack1Sound != null)
        {
            audioSource.PlayOneShot(Swordattack1Sound);
        }
    }
    ...
    {
        if (Swordattack2Sound != null)
        {
            audioSource.PlayOneShot(Swordattack2Sound);
        }
    }
